...
# Test naive CTMC (new method, modified by HSB, 8/10/18)
def test_CTMC(ts, tseq, ts_test, tseq_test, wantST=True, md="naive"):
    ns = len(ts_test)
    ...
    # baseline: zero out columns corresponding to dead-end states, i.e.,
    # make it impossible to transition into those states
    if md=="baseline1":
        for s in des:
            phat[:,s] = 0.0
            phat[s,0] = 1.0e-6
        for i in range(numstates):
            phat[i,i] = 0.0
            phat[i,i] = -np.sum(phat[i,:])

    # baseline: zero out columns but retain probabilities
    if md=="baseline2":
        skeleton = np.copy(phat)
        for i in range(numstates):
            if i not in des:
                skeleton[i,:] /= -skeleton[i,i]
                skeleton[i,i] = 0.0
        for s in des:
            skeleton[:,s] = 0.0
        for i in range(numstates):
            if i not in des:
                skeleton[i,:] /= np.sum(skeleton[i,:])
                skeleton[i,:] *= (-phat[i,i])
                skeleton[i,i] = phat[i,i]
        phat = skeleton
        for s in des:
            phat[s,0] = 1.0e-6
            phat[s,s] = -1.0e-6

    # baseline: figure out which state holds on the longest
    # and just return to that state whenever we hit a dead end
    if md=="baseline3":
        longstate = np.argmin(np.diag(-phat))    
        for s in des:
            phat[s,longstate] = 1.0e-6
            phat[s,s] = -1.0e-6

    # baseline: figure out which state holds on the shortest
    # and just return to that state whenever we hit a dead end
    if md=="baseline4":
        shortstate = np.argmax(np.diag(-phat))    
        for s in des:
            phat[s,shortstate] = 1.0e-6
            phat[s,s] = -1.0e-6
