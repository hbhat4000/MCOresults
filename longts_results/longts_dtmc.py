...
for i in states:
    ...
    for j in T:
        ...
        ts[L] = i-1
        ts_test[j] = i - 1
        LTerr, LTerr_test,_ = test_naiveDTMC(ts,ts_test,wantST=False)
        LTerr_fixed, LTerr_test_fixed,constrviol,_,sparsity= test_fixedDTMC(ts,ts_test,wantST=False)
...
train = train.rename_axis('# of states').rename_axis('DTMC simulation',axis='columns')
fixed = pd.DataFrame(train_err_fixed,index=states,columns=ind)
fixed = fixed.rename_axis('# of states').rename_axis('DTMC simulation',axis='columns')
...
naive_dtmc = {}
fixed_dtmc = {}
naive_dtmc['training_err'] = train_err
naive_dtmc['test_err'] = err
fixed_dtmc['training_err'] = train_err_fixed
fixed_dtmc['test_err'] = err_fixed
io.savemat('lts_naive_dtmc',naive_dtmc)
io.savemat('lts_fixed_dtmc',fixed_dtmc)
